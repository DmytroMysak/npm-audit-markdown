...
describe('generateAndSaveReportFile', () => {
  ...
  it('should throw error when nom audit was run with fix option', async () => {
    try {
      ...
      expect(error.message).to.equal(
        `Sorry! You can't use ${chalk.underline('npm audit fix')} with @netly/npm-audit-markdown.`
      );
    }
  });
  ...
  it('should generate proper markdown file for v6 npm audit report', async () => {
    mockFs({});
    const expectedResult = mockFs.bypass(() =>
      readFileSync(`${__dirname}/expected-results/npm-audit-6-result.md`, 'utf8')
    );
  ...
  it('should generate proper markdown file for clean v6 npm audit report', async () => {
    mockFs({});
    const expectedResult = mockFs.bypass(() =>
      readFileSync(`${__dirname}/expected-results/npm-audit-clean-6-result.md`, 'utf8')
    );
  ...
  it('should generate proper markdown file for v7 npm audit report', async () => {
    mockFs({});
    const expectedResult = mockFs.bypass(() =>
      readFileSync(`${__dirname}/expected-results/npm-audit-7-result.md`, 'utf8')
    );
  ...
  it('should generate proper markdown file for clean v6 npm audit report', async () => {
    mockFs({});
    const expectedResult = mockFs.bypass(() =>
      readFileSync(`${__dirname}/expected-results/npm-audit-clean-7-result.md`, 'utf8')
    );
